...
GOOGLE_API_KEY = input('Ingresa tu API Key de Google Gemini (válida, obtenla en https://aistudio.google.com/app/apikey): ')
...
regex_cedula = r"(?:CC|C\.C|Identificaci[oó]n|NIT)[:\s]*(\d{6,10})\b"

regex_nombre_labeled = r"(?:Nombre|Cotizante|Afiliado|Razon Social|Cliente|EMPLEADO|APORTANTE|Nombres)[:\s]*([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ ]*(?:[ \t][A-ZÁÉÍÓÚÑa-záéíóúñ ]+)*)(?=\s*\n|IBC|VALOR|Periodo|Total|Identificaci[oó]n|\d{6,10}|MES|SALUD|PENSION|RIESGOS|APORTE|$)"
...
regex_ibc = r"(?:IBC|Base de Cotizaci[oó]n|Salud\s+IBC|PENSION\s+IBC)[:\s]*(?:\$?\s*)?(" + numeric_pattern + r")"
...
regex_mora = r"(?:Mora|Intereses Mora|INTERESES MORA|Total Int[eé]reses Mora)[:\s]*(?:\$?\s*)?(" + numeric_pattern + r")"
...
def limpiar_ia_valor(value):
    """Limpia y estandariza los valores numéricos extraídos por la IA."""
    if not value:
        return ""
...
def extraer_campos_con_ia(texto_pdf):
    ...
    prompt = f"""Dada el siguiente texto de un documento PDF, extrae la Cédula (entre 6 y 10 dígitos), el Nombre completo de la persona/empresa (puede estar etiquetado como Nombre, Cotizante, Afiliado, Razon Social, Cliente, Empleado o Aportante), el Mes de cotización (solo el nombre del mes, ej. 'Octubre'), el IBC (Ingreso Base de Cotización), el valor de Salud, el Valor Sin Mora y el Valor Con Mora. Si un campo no se encuentra, deja su valor como una cadena vacía. Los valores numéricos pueden usar punto como separador de miles y coma como separador decimal. Devuelve la información como un objeto JSON con las siguientes claves: 'Cédula', 'Nombre', 'Mes', 'IBC', 'Salud', 'Valor Sin Mora', 'Valor Con Mora'.\n\nTexto:\n\n{texto_pdf}"""
    ...
    try:
        ...
        if response.text:
            ...
            cedula = extracted_data.get('Cédula', '')
...
def extraer_campos(texto, tablas_extraidas=None):
    ...
    # If Cédula is not found by IA, try with regex
    if not cedula:
    ...
    if not nombre:
        ...
        if not nombre and cedula:
            n_after_cc_pattern = re.search(
                r"(?:CC|C\.C|Identificaci[oó]n)\s*" + re.escape(cedula) + r"\s*\n?\s*([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ ]*(?:[ \t][A-ZÁÉÍÓÚÑa-záéíóúñ ]+)*)(?=\s*\n|IBC|VALOR|Periodo|Total|Identificaci[oó]n|\d{6,10}|MES|SALUD|PENSION|RIESGOS|APORTE|$)",
                texto,
            ...
            if not nombre:
                 n_same_line_after_cc_pattern = re.search(
                    r"(?:CC|C\.C|Identificaci[oó]n)\s*" + re.escape(cedula) + r"\s*([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ ]*(?:[ \t][A-ZÁÉÍÓÚÑa-záéíóúñ ]+)*)",
                    texto,
                    re.IGNORECASE
                )
...
def process_pdfs_and_display_df():
    ...
    print("Todos los PDFs han sido procesados y los datos extraídos.")
    print(f"Total de registros extraídos: {len(datos_extraidos)}")

    df = pd.DataFrame(datos_extraidos)
    print("DataFrame creado con los datos extraídos:")
    display(df)
...
bn_subir = widgets.Button(description="📤 Subir PDFs", button_style='info')
...
bn_borrar = widgets.Button(description="🧹 Borrar PDFs", button_style='danger')
...
def borrar_archivos_handler(b):
    with out:
        ...
        if os.path.exists("pdfs"):
            ...
            print("Carpeta '/pdfs' eliminada y recreada vacía.")
        else:
            print("No existe la carpeta '/pdfs'.")

bn_borrar.on_click(borrar_archivos_handler)

# BOTÓN: PROCESAR Y GENERAR TABLA
bn_procesar = widgets.Button(description="⚙ Procesar PDFs y Generar Tabla", button_style='success')
...
display(HTML("<h3>Bienvenido a tu herramienta de extracción de datos de PDFs.</h3>"))
display(HTML("1. Haz clic en '📤 Subir PDFs' para cargar tus archivos (puedes subir varios a la vez).<br>"))
display(HTML("2. Luego, haz clic en '⚙ Procesar PDFs y Generar Tabla' para extraer los datos y ver la tabla.<br>"))
display(HTML("3. La tabla generada se guardar\u00e1 autom\u00e1ticamente como CSV, con un enlace para descargarla.<br>"))
display(HTML("4. Usa '🧹 Borrar PDFs' si quieres eliminar los archivos cargados.<br><br>"))
